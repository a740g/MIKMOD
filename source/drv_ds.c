...
SWORD DS_SampleLoad(/*SAMPLOAD* sload, int type*/FILE* fp, ULONG length, ULONG loopstart, ULONG loopend, UWORD flags)
{
	int    handle, t;
	SWORD* lpSampMem;
	ULONG  lengthAlloc;
	...
	if (!(lpSampMem = MyCalloc(lengthAlloc, 1)))
	{
	...
	// TODO: Why are we doing this for DS?
	...
	SL_Load(lpSampMem, length);	// note: length is now x2 
...
void DS_VoicePlay(UBYTE voice, SWORD handle, int start, ULONG size, ULONG reppos, ULONG repend, UWORD flags)
{
...
void DS_VoiceStop(UBYTE voice)
{
	ghld[voice].active = 0;
}


BOOL DS_VoiceStopped(UBYTE voice)
{
